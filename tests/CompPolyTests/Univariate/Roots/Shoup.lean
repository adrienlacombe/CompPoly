...
public meta import CompPoly.Fields.Binary.Tower.Impl
public meta import CompPoly.Univariate.Roots.Shoup
...
private def bt0ShoupCtx : SmallPrimeTraceContext BT0 where
  ...
  frobenius_fixed := by
    ...
    have hcard : Fintype.card BT0 = 2 := by
      simpa [BT0] using (ConcreteBinaryTower.getBTFResult 0).fieldFintypeCard
    rwa [hcard] at h
  p := 2
  k := 1
  p_prime := by decide
